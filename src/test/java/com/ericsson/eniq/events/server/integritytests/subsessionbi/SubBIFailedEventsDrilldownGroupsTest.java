...
import static com.ericsson.eniq.events.server.test.common.ApplicationTestConstants.SAMPLE_IMSI_GROUP;
...
public class SubBIFailedEventsDrilldownGroupsTest extends
    ...
    public void onSetUp() throws Exception {
        ...
        createAndPopulateIMSIGroupTable();
        createAndPopulateRawTables();
        createTemporaryTables();
    }

    private void createAndPopulateIMSIGroupTable() throws Exception {
        final Collection<String> columnsForIMSIGroupTable = new ArrayList<String>();
        columnsForIMSIGroupTable.add(IMSI);
        columnsForIMSIGroupTable.add(GROUP_NAME);
        createTemporaryTable(TEMP_GROUP_TYPE_E_IMSI, columnsForIMSIGroupTable);
    ...
    @Test
    public void testSubBIFailedEvents_IMSIGroup_ATTACH() throws Exception {
        final List<SubBIFailuresDrilldownResult> results = runQuery(ATTACH + COMMA + ATTACH_IN_2G_AND_3G);
    ...
    private void validateResultsFromSGEHTable(final List<SubBIFailuresDrilldownResult> results) {
        ...
        assertThat(activateResult.getIMSI(), is(testIMSI));
        assertThat(activateResult.getPTMSI(), is(Integer.toString(SAMPLE_PTMSI_VALUE)));

    }

    @Test
    public void testSubBIFailedEvents_IMSIGroup_L_HANDOVER() throws Exception {
        final List<SubBIFailuresDrilldownResult> results = runQuery(L_HANDOVER + COMMA + HANDOVER_IN_4G);
        validateResultsFromLTETable(results);
    }

    private List<SubBIFailuresDrilldownResult> runQuery(final String eventIdForNodeParam) throws URISyntaxException,
        ...
        map.putSingle(NODE_PARAM, eventIdForNodeParam);
    ...
    private void createAndPopulateRawTables() throws Exception {
        ...
        valuesToInsertInLTETable.put(EVENT_ID, HANDOVER_IN_4G);
        valuesToInsertInLTETable.put(RAT, RAT_FOR_LTE);
        valuesToInsertInLTETable.put(IMSI, testIMSI);
        ...
        valuesToInsertInSGEHTable.put(IMSI, testIMSI);
