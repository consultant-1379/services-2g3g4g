...
public class SubBIFailedEventsSuccessRawWithPreparedTablesTest extends TestsWithTemporaryTablesBaseTestCase<SubBIResult> {
    ...
    @Test
    public void testSubBIFailedEventsIMSI_5Minutes() throws Exception {
        jndiProperties.useSucRawJNDIProperty();
        populateTemporaryTables(DateTimeUtilities.getDateTimeMinus5Minutes());
    ...
    @Test
    public void testSubBIFailedEventsPTMSI_5Minutes() throws Exception {
        jndiProperties.useSucRawJNDIProperty();
        populateTemporaryTables(DateTimeUtilities.getDateTimeMinus5Minutes());
        final MultivaluedMap<String, String> map = new MultivaluedMapImpl();
        map.putSingle(DISPLAY_PARAM, CHART_PARAM);
    ...
    @Test
    public void testSubBIFailedEventsMSISDN_5Minutes() throws Exception {
        jndiProperties.useSucRawJNDIProperty();
        populateTemporaryTables(DateTimeUtilities.getDateTimeMinus5Minutes());
    ...
    @Test
    public void testSubBIFailedEventsIMSIGroup_5Minutes() throws Exception {
        jndiProperties.useSucRawJNDIProperty();
        populateTemporaryTables(DateTimeUtilities.getDateTimeMinus5Minutes());
    ...
    private void insertRow(final SQLExecutor sqlExecutor, final String table, final int eventID, final String datetimeID, final long imsi,
            final String ptmsi, final String date, final long msisdn) throws SQLException {
     final String localDateID = datetimeID.substring(0, 10);
     sqlExecutor.executeUpdate("insert into " + table + " values('" + date + "'," + eventID + "," + imsi + ",'" + localDateID + "', " + msisdn + ",'" + datetimeID + "'," + ptmsi + ")");
}
    ...
    private void populateTemporaryTables(final String dateTime) throws SQLException {
        ...
        try {
            ...
            insertRow(sqlExecutor, TEMP_EVENT_E_SGEH_ERR_RAW, 1, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_SGEH_ERR_RAW, 2, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_SGEH_ERR_RAW, 3, dateTime, testIMSI, testPTMSI, date, testMSISDN);

            insertRow(sqlExecutor, TEMP_EVENT_E_SGEH_SUC_RAW, 1, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_SGEH_SUC_RAW, 2, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_SGEH_SUC_RAW, 3, dateTime, testIMSI, testPTMSI, date, testMSISDN);

            insertRow(sqlExecutor, TEMP_EVENT_E_LTE_ERR_RAW, 5, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_LTE_ERR_RAW, 6, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_LTE_ERR_RAW, 7, dateTime, testIMSI, testPTMSI, date, testMSISDN);

            insertRow(sqlExecutor, TEMP_EVENT_E_LTE_SUC_RAW, 5, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_LTE_SUC_RAW, 6, dateTime, testIMSI, testPTMSI, date, testMSISDN);
            insertRow(sqlExecutor, TEMP_EVENT_E_LTE_SUC_RAW, 7, dateTime, testIMSI, testPTMSI, date, testMSISDN);
