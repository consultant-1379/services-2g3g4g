...
import com.ericsson.eniq.events.server.test.queryresults.KPIAnalysisByCauseCodeSgsnResult;
...
/**
 * Created by eeikbe on 22/09/2014.
 */
public class KPIAnalysisByCauseCodeSgsnTest  extends TestsWithTemporaryTablesBaseTestCase<KPIAnalysisByCauseCodeSgsnResult> {
    ...
    private void validateResultForKPIAnalysisByCauseCodeQuery(final String json) throws Exception {
        final List<KPIAnalysisByCauseCodeSgsnResult> results = getTranslator().translateResult(json, KPIAnalysisByCauseCodeSgsnResult.class);
        assertThat(results.size(), is(1));
        final KPIAnalysisByCauseCodeSgsnResult result = results.get(0);
        assertThat(result.getSgsnMme(), is(SAMPLE_SGSN));
