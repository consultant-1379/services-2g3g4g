...
public class KPIRatioTypeAPNDrillTypeBscTest extends TestsWithTemporaryTablesBaseTestCase<KPIByAPNFromBSCResult> {
    ...
    @Test
    public void testTypeAPNDrilltypeSGSNWithImsiAggOneWeek() throws Exception {
        jndiProperties.disableSucRawJNDIProperty();
        createImsiAggregationTables();
        createTopologyTables();
        populateTopologyForSgehQuery();
        ...
        populateSuccessImsiAggregationTablesForSgehQuery(timestamp);
        createAndPopulateRawTablesForLteQuery(timestamp, 8, 10, localDateId);
        final String result = runQueryWithNineHourOffset(SAMPLE_MME, TAU_IN_4G, ONE_WEEK, ERICSSON, SAMPLE_ERBS, Integer.toString(RAT_FOR_LTE));
        validateResultForLTEQuery(result);
    }
    @Test
    ...
    private String runQueryWithNineHourOffset(final String sgsnOrMME, final int eventId, final String time, final String vendor, final String hier3, final String rat)
            throws URISyntaxException {
        final MultivaluedMapImpl map = new MultivaluedMapImpl();
        map.putSingle(TYPE_PARAM, TYPE_APN);
        map.putSingle(DISPLAY_PARAM, GRID_PARAM);
    ...
    private void createAndPopulateRawTablesForLteQuery(final String timestamp, final int noOfErrorRowsInRaw, final int noOfSuccessRowsInRaw, final String localDateId)
            throws Exception {
    ...
    private void createAndPopulateRawTablesForSgehQuery(final String timestamp, final int noOfErrorRowsInRaw, final int noOfSuccessRowsInRaw, final String localDateId)
