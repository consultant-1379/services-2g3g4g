...
public class EventAnalysisSummaryWithPreparedTablesApnTest extends
    ...
    private void createAndPopulateRawTables(final String timestamp) {
        try {
            final String localDateId = dateOnlyFormatter
                    .format(dateOnlyFormatter.parse(timestamp));
            setupRawTableValues(timestamp, localDateId, SAMPLE_APN,
                    ATTACH_IN_2G_AND_3G, RAT_FOR_GSM, 0, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN);
            ...
            setupRawTableValues(timestamp, localDateId, SAMPLE_APN, ATTACH_IN_2G_AND_3G, RAT_FOR_GSM, 0, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN);
            valuesForSgehTable.putAll(columnsInSgehRawTables);
            insertRow(TEMP_EVENT_E_SGEH_ERR_RAW, valuesForSgehTable);

            setupRawTableValues(timestamp, localDateId, SAMPLE_APN,
                    ATTACH_IN_4G, RAT_FOR_LTE, 0, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN);
            ...
            setupRawTableValues(timestamp, localDateId, SAMPLE_APN,
                    ATTACH_IN_4G, RAT_FOR_LTE, 1, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN, TEST_CAUSE_VALUE_KNOWN);
    ...
    public void testGetDetailedData_APN_1Week() throws Exception {
        ...
        final List<EventAnalysisDetailedResult> summaryResult = getTranslator()
                .translateResult(json, EventAnalysisDetailedResult.class);
        assertThat(summaryResult.size(), is(4));
    }

    private MultivaluedMap<String, String> getRequestParametersForAPNFailuresDrill(
        ...
        map.putSingle(EVENT_ID_PARAM, Integer.toString(ATTACH_IN_2G_AND_3G));
        return map;
    }

    @Test
    public void testGetDetailedData_APN_DrillOnFailures_1Week()
            throws Exception {

        createAndPopulateRawTables(DateTimeUtilities.getDateTimeMinus48Hours());
        final MultivaluedMap<String, String> map = getRequestParametersForAPNFailuresDrill(ONE_WEEK);
        DummyUriInfoImpl.setUriInfo(map, eventAnalysisService);
        final String json = getData(eventAnalysisService, map);
        final List<EventAnalysisDetailedResult> summaryResult = getTranslator()
                .translateResult(json, EventAnalysisDetailedResult.class);
    ...
    @Test
    public void testGetDetailedData_APN_DrillOnFailures_30Minutes()
            throws Exception {

        createAndPopulateRawTables(DateTimeUtilities.getDateTime(
                Calendar.MINUTE, -30));
        final MultivaluedMap<String, String> map = getRequestParametersForAPNFailuresDrill(THIRTY_MINUTES);
        DummyUriInfoImpl.setUriInfo(map, eventAnalysisService);
        final String json = getData(eventAnalysisService, map);
        final List<EventAnalysisDetailedResult> summaryResult = getTranslator()
                .translateResult(json, EventAnalysisDetailedResult.class);
    ...
    @Test
    public void testGetDetailedData_APN_DrillOnFailures_6Hours()
            throws Exception {

        createAndPopulateRawTables(DateTimeUtilities.getDateTime(
                Calendar.MINUTE, -120));
        final MultivaluedMap<String, String> map = getRequestParametersForAPNFailuresDrill(SIX_HOURS);
        DummyUriInfoImpl.setUriInfo(map, eventAnalysisService);
        final String json = getData(eventAnalysisService, map);
        final List<EventAnalysisDetailedResult> summaryResult = getTranslator()
                .translateResult(json, EventAnalysisDetailedResult.class);
    ...
    @Test
    public void testGetDetailedData_APN_DrillOnFailures_1Day() throws Exception {

        createAndPopulateRawTables(DateTimeUtilities.getDateTime(
                Calendar.MINUTE, -320));
        final MultivaluedMap<String, String> map = getRequestParametersForAPNFailuresDrill(ONE_DAY);
        DummyUriInfoImpl.setUriInfo(map, eventAnalysisService);
        final String json = getData(eventAnalysisService, map);
        final List<EventAnalysisDetailedResult> summaryResult = getTranslator()
                .translateResult(json, EventAnalysisDetailedResult.class);
    ...
    private void setupRawTableValues(final String timestamp,
            final String localDateId, final String apn, final int eventId,
            final int rat, final int valueForQCIErr1, final String causeCode, final String subCauseCode, final String causeProtType) {
    ...
    private void insertRowIntoCCTable(final String table, final int causeCode,
            final int causeProtoType, final String ccDesc) throws SQLException {
    ...
    private void insertRowIntoSCCTable(final String table,
            final int subCauseCode, final String sccDesc) throws SQLException {
        final Map<String, Object> valuesForTable = new HashMap<String, Object>();
