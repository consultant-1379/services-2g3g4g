...
import com.ericsson.eniq.events.server.test.queryresults.KPIAnalysisByCauseCodeCellResult;
...
/**
 * Created by eeikbe on 12/09/2014.
 */
public class KPIAnalysisByCauseCodeCellTest  extends TestsWithTemporaryTablesBaseTestCase<KPIAnalysisByCauseCodeCellResult> {
    ...
    public void testKPIAnalysisByCauseCode() throws Exception {
        ...
        final String result = runQueryKPIAnalysisByCauseCode(SAMPLE_CELL, SAMPLE_BSC, RAT_FOR_3G, ISRAU_IN_2G_AND_3G, ONE_WEEK);
        validateResultForKPIAnalysisByCauseCodeQuery(result);
    }

    @Test
    public void testKPIAnalysisByCauseCodeBsc() throws Exception {
        final String timestamp = DateTimeUtilities.getDateTimeMinusDay(7);
        final String localDateId = dateOnlyFormatter.format(dateOnlyFormatter.parse(timestamp));
        createAndPopulateEVENT_E_SGEH_ERR_RAW(CAUSE_CODE_VALUE, SUB_CAUSE_CODE_VALUE, CAUSE_PROT_TYPE_VALUE, timestamp, localDateId);
        createAndPopulateEVENT_E_LTE_ERR_RAW(CAUSE_CODE_VALUE, SUB_CAUSE_CODE_VALUE, CAUSE_PROT_TYPE_VALUE, timestamp, localDateId);
        createAndPopulateDIMTables();
        final String result = runQueryKPIAnalysisByCauseCode_Bsc(SAMPLE_CELL, SAMPLE_BSC, RAT_FOR_3G, ISRAU_IN_2G_AND_3G, ONE_WEEK);
        validateResultForKPIAnalysisByCauseCodeQuery(result);
    }
    ...
    private String runQueryKPIAnalysisByCauseCode_Bsc(final String cell, final String bsc, final int rat, final int eventId, final String time) throws URISyntaxException {
    ...
    private String runQueryKPIAnalysisByCauseCode_Sgsn(final String cell, final String bsc, final int rat, final int eventId, final String time) throws URISyntaxException {
        final MultivaluedMapImpl map = new MultivaluedMapImpl();
        map.putSingle(TIME_QUERY_PARAM, time);
        map.putSingle(DISPLAY_PARAM, GRID_PARAM);
        map.putSingle(TZ_OFFSET, "+0100");
    ...
    private String runQueryKPIAnalysisByCauseCode(final String cell, final String bsc, final int rat, final int eventId, final String time) throws URISyntaxException {
        final MultivaluedMapImpl map = new MultivaluedMapImpl();
        map.putSingle(TIME_QUERY_PARAM, time);
        map.putSingle(DISPLAY_PARAM, GRID_PARAM);
        map.putSingle(TZ_OFFSET, "+0100");
    ...
    private void validateResultForKPIAnalysisByCauseCodeQuery(final String json) throws Exception {
        final List<KPIAnalysisByCauseCodeCellResult> results = getTranslator().translateResult(json, KPIAnalysisByCauseCodeCellResult.class);
        assertThat(results.size(), is(1));
        final KPIAnalysisByCauseCodeCellResult result = results.get(0);
        //01. RAT ID
        ...
        //05. Cause Code ID
        assertThat(result.getCauseCodeId(), is(CAUSE_CODE_VALUE));
        //06. SubCauseCodeID
        assertThat(result.getSubCauseCodeId(), is(SUB_CAUSE_CODE_VALUE));
        //07. Cause Prot Type
